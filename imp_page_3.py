...
df_axe = (data.groupby('AXE_PRODUIT').size()
          .reset_index(name="Nombre d'emplacements")
          .sort_values(by="Nombre d'emplacements",
                       ascending=False))

df_axe_r = (df_with_reserve.groupby('AXE_PRODUIT').size()
            .reset_index(name="Nombre d'emplacements")
            .sort_values(by="Nombre d'emplacements",
                         ascending=False))
...
if show_table:
    ...
    lettre_order = ['1','A','B','C',
                    ...
                    'T','U','V','W',
                    'X','Y','Z','NEX',
                    'DYN','FP','OCC','PAS','PARFUM','SOIN','MAQUILLAGE','S+'] 
    ...
    df_histo['Lettre | Axe Réserves'] = pd.Categorical(df_histo['Lettres'], categories=lettre_order, ordered = True)
    df_histo = df_histo[df_histo['Lettre | Axe Réserves'].notna()]
    


    valeurs_uniques = sorted(set(df_histo["Lettre | Axe Réserves"]))
    couleur_par_valeur = {}
    ...
    df_histo,
    x="Lettres",
    color="Lettre | Axe Réserves",
    color_discrete_map=couleur_par_valeur,
    text_auto = True,
    title=(f"📊 Fréquence des Lettres et / ou des Axes Réserves dans l'Allée {allsts_input}"),
